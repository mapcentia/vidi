...
var yell = function (obj, override = false) {
  ...
  if (debug) {
    console.log(
      `[${dateTime}] [ex:GeoSag] : ${JSON.stringify(obj, undefined, 2)}`
    );
  } else if (override) {
    console.log(
      `[${dateTime}] [ex:GeoSag] : ${JSON.stringify(obj, undefined, 2)}`
...
GC2_HOST =
  GC2_HOST.split("https://").length > 1
    ? GC2_HOST.split("https://")[1]
    : GC2_HOST;
...
function verifyUser(request) {
  return new Promise(function (resolve, reject) {
    ...
    if (
      dn.allow_from.includes(request.headers["x-forwarded-for"]) ||
      request.connection.remoteAddress == "::1"
    ) {
      resolve({ success: true, message: "User allowed" });
    } else {
      yell(
        `Blocked access for ${request.connection.remoteAddress}/${request.headers["x-forwarded-for"]}`,
        true
      );
...
function matrikelExists(matr) {
  ...
  return new Promise(function (resolve, reject) {
    getDocunote(
      `Persons/synchronizeSource/${dn.synchronizeSource}/synchronizeId/${matr.key}`,
      "GET"
    ).then((r) => {
      resolve({
...
function getConnectedCases(partNodeType, partNodeId) {
  ...
  return new Promise(function (resolve, reject) {
    getDocunote(
      `Cases/pickername/${dn.partsPicker}/partNodeType/${partNodeType}/partRecordId/${partNodeId}`
    )
      .then((r) => {
        resolve(r);
      })
  ...
  try {
    ...
    verifyUser(req)
      ...
      .then(function (exists) {
        ...
        if (
          exists.partNodeType == undefined ||
          exists.partRecordId == undefined
        ) {
