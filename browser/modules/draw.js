...
module.exports = {
    ...
    if (enable && !drawControl) {
      ...
      cloud.get().map.on("draw:created", function (e) {
        ...
        if (type === "circlemarker") {
          ...
          var text = prompt(
            __("Enter a text for the marker or cancel to add without text"),
            ""
          );
          if (text !== null) {
            drawLayer
      ...
      cloud.get().map.on("draw:edited", function (e) {
        $.each(e.layers._layers, function (i, v) {
          if (typeof v._mRadius !== "undefined") {
            v.feature.properties.distance = L.GeometryUtil.readableDistance(
              v._mRadius,
              true
            );
      ...
      var po1 = $(".leaflet-draw-section:eq(0)").popover({
        content: __(
          "Use these tools for creating markers, lines, areas, squares and circles."
        ),
        placement: "left",
      });
      po1.popover("show");
      ...
      $.each(v[0].geojson.features, function (n, m) {
        ...
        if (m.type === "CircleMarker") {
          ...
          if (m._vidi_marker_text) {
            g.bindTooltip(m._vidi_marker_text, { permanent: true })
              .on("click", () => {})
              .openTooltip();
          }
        ...
        if (m.type === "Marker") {
          ...
          if (m._vidi_extremities) {
            g.showExtremities(
              m._vidi_extremities.pattern,
              m._vidi_extremities.size,
              m._vidi_extremities.where
            );
          }
  ...
  bindPopup: function (event) {
    ...
    popup
      ...
      .setContent(
        '<p style="width: 200px">' +
          __("Apply default style settings for this drawing?") +
          '</p><a href="javascript:void(0)" id="btn-draw-apply-style-cancel" class="btn btn-raised btn-default btn-xs">' +
          __("Cancel") +
          '</a><a href="javascript:void(0)" id="btn-draw-apply-style-ok" class="btn btn-raised btn-primary btn-xs">' +
          __("Ok") +
          "</a>"
      )
  ...
  setStyle: function (l, type) {
    ...
    if (type === "polyline") {
      l.showExtremities(
        $("#draw-line-extremity").val(),
        $("#draw-line-extremity-size").val(),
        $("#draw-line-extremity-where").val()
      );
...
(function () {
  ...
  var PolylineExtremities = {
    ...
    showExtremities: function (pattern, size, where) {
      ...
      markersNode.setAttribute(
        "fill-opacity",
        markersNode.getAttribute("stroke-opacity")
      );
