...
module.exports = {
    ...
    init: function () {
        var base64name, arr, groups, metaData, i, l, count, displayInfo, tooltip, visibleLayers, checked;
        ...
        visibleLayers = layers.getLayers(";") ? layers.getLayers(";").split(";") : [];
                ...
                for (var u = 0; u < metaData.data.length; ++u) {
                    if (metaData.data[u].layergroup == arr[i]) {
                        ...
                        if (metaData.data[u].baselayer) {
                            ...
                            if (visibleLayers.indexOf(metaData.data[u].f_table_schema + "." + metaData.data[u].f_table_name) !== -1) {
                                checked = "checked";
                            } else {
                                checked = "";
                            }
                            ...
                            $("#collapse" + base64name).append('<li class="layer-item list-group-item"><div class="checkbox"><label class="overlay-label" style="width: calc(100% - 80px);"><input ' + checked + ' type="checkbox" id="' + metaData.data[u].f_table_name + '" data-gc2-id="' + metaData.data[u].f_table_schema + "." + metaData.data[u].f_table_name + '"><span>' + text + (metaData.data[u].authentication === "Read/write" ? " <i class=\"fa fa-lock gc2-session-lock\" aria-hidden=\"true\"></i>":"") + '</span></label><span></span><span data-gc2-key="' + metaData.data[u].f_table_schema + "." + metaData.data[u].f_table_name + "." + metaData.data[u].f_geometry_column + '" data-toggle="tooltip" data-placement="left" title="Add new feature to layer" class="edit-label label label-info gc2-add-feature gc2-edit-tools">New</span><span data-toggle="tooltip" data-placement="left" title="' + tooltip + '" style="visibility: ' + displayInfo + '" class="info-label label label-primary" data-gc2-id="' + metaData.data[u].f_table_schema + "." + metaData.data[u].f_table_name + '">Info</span></div></li>');
                            l.push({});
                        }
                    }
                }
