    ...
    init: function (onLoad, el, onlyAddress, getProperty) {
        ...
        $("#" + el).typeahead({
            ...
            },
            {
                ...
                displayKey: 'value',
                templates: {
                    header: '<h2 class="typeahead-heading">Rammeområde</h2>'
                },
                source: function (query, cb) {
                    var names = [];
                    (function ca() {
                        ...
                        dslB = {
                            ...
                            "query": {
                                "bool": {
                                    "must": {
                                        "query_string": {
                                            "default_field": "properties.enrid",
                                            "query": query.toLowerCase(),
                                            "default_operator": "AND"
                                        }
                                    }
                        ...
                        $.ajax({
                            url: 'https://cowi.mapcentia.com/api/v2/elasticsearch/search/esbjerg/kommuneplan18/kpplandk2_view',
        ...
        $('#' + el).bind('typeahead:selected', function (obj, datum, name) {
            if ((type1 === "adresse" && name === "adresse") || (type2 === "jordstykke" && name === "matrikel")
                || (type3 === "esr_nr" && name === "esr_ejdnr") || (type4 === "sfe_nr" && name === "sfe_ejdnr") || (name === "enrid")) {
                placeStore.reset();
                switch (name) {
                    case "enrid" :
                        placeStore.db = "esbjerg";
                        placeStore.host = "https://cowi.mapcentia.com";
                        searchString = datum.value;
                        placeStore.sql = "SELECT gid,planid,the_geom,ST_asgeojson(ST_transform(the_geom,4326)) as geojson FROM kommuneplan18.kpplandk2_view WHERE enrid='" + gids[datum.value] + "'";
                        placeStore.load();
                        break;
                    case "esr_ejdnr" :
                        placeStore.db = MDB;
